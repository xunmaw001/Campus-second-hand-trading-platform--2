...
//                            goodsOrderEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R liuyan(Integer id, String liuyanText, Integer goodsliuyanPingfenNumber, HttpServletRequest request){
        logger.debug("liuyan方法:,,Controller:{},,id:{}",this.getClass().getName(),id);
        GoodsOrderEntity goodsOrder = goodsOrderService.selectById(id);
        if(goodsOrder == null)
        ...
        goodsliuyanEntity.setId(id);
        goodsliuyanEntity.setGoodsId(goodsId);
        goodsliuyanEntity.setYonghuId((Integer) request.getSession().getAttribute("userId"));
        goodsliuyanEntity.setGoodsLiuyanText(liuyanText);
        goodsliuyanEntity.setInsertTime(new Date());
        goodsliuyanEntity.setCreateTime(new Date());
        goodsLiuyanService.insert(goodsliuyanEntity);
